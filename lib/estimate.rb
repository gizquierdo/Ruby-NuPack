...
class Estimate
	attr_reader :base_price, :flat_charge, :labour_quantity, :labour_charged, :materials_charged_hash, :date_estimate_given, :employee_ID
	...
	def get_final_price
		...
		return (@base_price + @flat_charge + @labour_charged + total_mat_markup).round(2)
	end
		
	#This function will return a json object
	def get_price_breakdown_string
		return "Base Price: " + @base_price.to_s + ", Flat charge: " + @flat_charge.to_s + ", Charged " + @labour_charged.to_s + " for " + @labour_quantity.to_s + " workers. Materials " +  materials_hash_to_s + " TOTAL: " + get_final_price.to_s
	end
